...
class Portfolio extends CI_Controller {
	...
	public function getvalues($Get){
		...
		if($Get == 'table'){
			...
			$return	=	'assets/upload/projects/';
		} else if($Get == 'Headings'){
			$return =  array('Portfolio Name', 'Description', 'SortOrder');
		} else if($Get == 'TableHeading'){
			$return =  array('PortfolioTitle', 'Description', 'SortOrder');
		}
		return $return;
	}
	...
	public function getfielddata($ID = NULL){
		    ...
		    $Completed		=	$this->input->post('CompletedTime');
		    ...
		    $Progress		=	$this->input->post('Progress');
	  		...
	  		$fields['Tags']	=	$this->getTAGS($Tags, $NewTags);
	  		...
	  		$iPhoneImage	=	$this->uploadImage('iPhoneImage', 'oldiPhoneImage', $loc);
	  		if($iPhoneImage != ''){
		  		$fields['iPhoneImage']	=	$iPhoneImage;
	  		}
	  		
	  		$LaptopImage	=	$this->uploadImage('LaptopImage', 'oldLaptopImage', $loc);
	  		if($LaptopImage != ''){
		  		$fields['LaptopImage']	=	$LaptopImage;
	  		}
