...
class KeyNotes extends CI_Controller {
	...
	public function getvalues($Get){
		...
		if($Get == 'table'){
			...
			$return =  array('Name', 'Description', 'SortOrder');
		} else if($Get == 'TableHeading'){
			$return =  array('Title', 'Description', 'SortOrder');
		}
		return $return;
	}
	...
	public function getfielddata(){
			...
			if (isset($_FILES['Image']) && $_FILES['Image']['tmp_name']){
					$config['upload_path']   = 'assets/upload/keynotes/';
					$config['allowed_types'] = '*';
					$config['max_size']		 = '10000000000';
					
			if(isset($_POST['oldImage'])){
				if(is_file('assets/upload/keynotes/'.$_POST['oldImage'])){
				unlink('assets/upload/keynotes/'.$_POST['oldImage']);
				}
			}
