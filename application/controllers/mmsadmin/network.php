...
class Network extends CI_Controller {
	...
	public function getvalues($Get){
		...
		if($Get == 'table'){
			...
			$return =  array('Country', 'Image', 'SortOrder');
		} else if($Get == 'TableHeadings'){
			$return =  array('Network', 'Image', 'SortOrder');
		}		
	...
	public function getfielddata($ID = NULL){
			...
			if (isset($_FILES['Image']) && $_FILES['Image']['tmp_name']){
					$config['upload_path']   = 'assets/upload/network/';
					$config['allowed_types'] = '*';
					$config['max_size']		 = '10000000000';
					
			if(isset($_POST['oldImage'])){
				if(is_file('assets/upload/network/'.$_POST['oldImage'])){
				unlink('assets/upload/network/'.$_POST['oldImage']);
				}
			}
