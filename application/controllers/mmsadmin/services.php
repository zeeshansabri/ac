...
class Services extends CI_Controller {
	...
	public function getfielddata(){
		    ...
		    $SDescription	=	$this->input->post('ShortDescription');
		    ...
		    $SEO			= 	preg_replace("/[^A-Za-z0-9 ]/", '', $Name);
  		    $SEO			=  	explode(" ", $SEO);
  		    $SEO			=	implode("-", $SEO);
			...
			$fields['Image'] = $this->do_upload($config, 'Image');	
  			...
  			if (isset($_FILES['InnerImage']) && $_FILES['InnerImage']['tmp_name']){
					$config['upload_path']   = 'assets/upload/services/';
					$config['allowed_types'] = '*';
					$config['max_size']		 = '10000000000';
					
			if(isset($_POST['oldInnerImage'])){
				if(is_file('assets/upload/services/'.$_POST['oldInnerImage'])){
				unlink('assets/upload/services/'.$_POST['oldInnerImage']);
				}
			}
			
			$fields['InnerImage'] = $this->do_upload($config, 'InnerImage');	
