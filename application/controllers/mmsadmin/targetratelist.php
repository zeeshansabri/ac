...
class TargetRateList extends CI_Controller {
	...
	public function getvalues($Get){
		...
		if($Get == 'table'){
			...
			$return =  array('Name', 'Code', 'Rate', 'SortOrder');
		} else if($Get == 'TableHeadings'){
			$return =  array('Country', 'Code', 'Rate', 'SortOrder');
		}		
	...
	public function view_all(){
		...
		/* ALL DATA */
		$RQ		 = 	' SELECT a.*, b.Country, c.Network FROM targetratelist AS a LEFT JOIN country AS b ON a.CountryID = b.ID LEFT JOIN network AS c ON a.NetworkID = c.ID';
		...
		public function edit($ID){
			...
			$RQ		 = 	' SELECT a.*, b.Country, c.Network FROM targetratelist AS a LEFT JOIN country AS b ON a.CountryID = b.ID LEFT JOIN network AS c ON a.NetworkID = c.ID WHERE a.ID = ' . $ID;
			$RQR	 =	$this->db->query($RQ);
			$result  = 	$RQR->result();
			$data['Info'] = $result;
