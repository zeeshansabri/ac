...
class SpecialOffers extends CI_Controller {
	...
	public function getvalues($Get){
		...
		if($Get == 'table'){
			...
			$return = 'Speical Offers';
		} else if($Get == 'Headings'){
			$return =  array('Name', 'Image', 'SortOrder');
	...
	public function getfielddata(){
			...
			if (isset($_FILES['Image']) && $_FILES['Image']['tmp_name']){
					$config['upload_path']   = 'assets/upload/specialoffer/';
					$config['allowed_types'] = '*';
					$config['max_size']		 = '10000000000';
					
			if(isset($_POST['oldImage'])){
				if(is_file('assets/upload/spcialoffer/'.$_POST['oldImage'])){
				unlink('assets/upload/specialoffer/'.$_POST['oldImage']);
				}
			}
